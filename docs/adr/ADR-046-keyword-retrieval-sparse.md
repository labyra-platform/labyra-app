...
> **Verified R238c (web_search Firestore docs):** single-field index với
> *collection-group scope* **không** được Firestore duy trì tự động → kể cả
> `collectionGroup('chunks').where('tenantId','==')` của ADR-033 P2 cũng **cần
> index khai tay + backfill `tenantId` lên chunk**, và muốn lọc status/group thì
> phải denorm `paperStatus`/`groupId` lên chunk (stale khi reprocess/archive/đổi
> group). Tức P2 không "miễn phí" — càng nghiêng về Pinecone sparse cho dài hạn.
...
Nhịp 1 (DONE — R237bq): T-4 fail-soft — retrieveBM25 timeout 5s → vector-only.
  + đọc T-7 search_timing log xác nhận bm25_retrieve là P95.   ← bảo vệ tức thời
   ↓
Nhịp 1.5 (DONE — R238c): Promise.all per-paper chunk reads (getCorpus +
  retrieveBM25) + vector‖BM25 song song. Xoá phần **N+1 round-trip** (1001 RTT
  tuần tự → ~1 RTT), đủ gỡ bottleneck ở quy mô hiện tại (~50 paper: ~2.5-3s →
  ~0.1-0.2s; phần lớn cost là RTT, không phải scoring). **Vẫn O(corpus)** về
  RAM/CPU (load + score toàn chunk mỗi query) — chỉ Nhịp 2 mới chữa tận gốc.
  Không schema/index/backfill → cầu nối no-regret trong lúc chờ số liệu/quy mô.
   ↓ (kích hoạt khi: tenant tiến gần >100 paper / ~10k+ chunk, HOẶC T-7 cho thấy
      scoring O(corpus) — không còn RTT — trở thành P95)
...
- **Gate chất lượng (bắt buộc trước cutover):** đổi backend keyword **đổi kết quả
  retrieval** → chạy `ragasEvalWeekly` (context precision/recall) A/B `bm25` vs
  `sparse` trên bộ truy vấn vàng; chỉ cutover khi sparse **không tụt** so với BM25
  hiện tại (chuẩn publication). Latency/cost xanh là **chưa đủ** để cam kết.
- **KHÔNG làm corpus-cache in-memory** (ý tưởng cache text chunk cạnh encoder để
  retrieveBM25 0-read/query): bị hướng Pinecone-sparse thay thế — đừng xây thứ sắp xoá.
...
*Document version 1.1 (R238c) — Proposed. Cập nhật: Nhịp 1.5 (Promise.all N+1 fix)
đã ship gỡ bottleneck RTT ở quy mô hiện tại; thêm gate RAGAS + finding index
collection-group. Next: số liệu T-7 P95 + ước cost → chuyển Accepted, lên lịch Nhịp 2.*
