/**
 * POST /api/dft/results — consolidated scientific summary for a workflow.
 * Auth + tenant required; proxies the worker /dft/results. @phase R297
 */
import { NextResponse } from 'next/server';
import { z } from 'zod';
import { getCurrentTenantId, getCurrentUser } from '@/lib/auth/server';
import { fetchDftResults } from '@/lib/dft/worker-client';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await fetchDftResults({ tenantId, workflowId: parsed.data.workflowId });
    if (!result.ok) {
