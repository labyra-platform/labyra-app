/**
 * API: PATCH/DELETE /api/materials/[id] — update / delete Material
 * @phase R160-data-1
 */
...
async function authorize(
  ...
  const rl = await checkRateLimit(rateLimitKey('materials-edit', tenantId), 30, 60);
  if (!rl.allowed) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const ref = db.doc(`tenants/${auth.tenantId}/materials/${id}`);
