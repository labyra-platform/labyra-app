/**
 * POST /api/measurements/[id]/reactivate — undo deprecation.
 *
 * @phase R164-phase-4b
 */
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  const rl = await checkRateLimit(rateLimitKey('measurements-reactivate', auth.tenantId), 10, 60);
  if (!rl.allowed) {
  ...
  try {
    await reactivateMeasurement(id, auth.tenantId, auth.uid);
    ...
    console.error('POST /api/measurements/[id]/reactivate', err);
