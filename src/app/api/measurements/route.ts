/**
 * /api/measurements — list + create measurements.
 *
 * @phase R164-phase-4b
 */
import { type NextRequest, NextResponse } from 'next/server';
import { authenticate, authenticateWriter } from '@/lib/api/auth-helper';
import { createMeasurement, listMeasurements } from '@/lib/firebase/measurements/service';
import { CreateMeasurementSchema } from '@/lib/schemas/measurement-schema';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const items = await listMeasurements(auth.tenantId, {
      includeDeprecated,
      includeRetracted,
      limit
    });
    ...
    console.error('GET /api/measurements', err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const item = await createMeasurement(parsed.data, {
      tenantId: auth.tenantId,
      createdBy: auth.uid
    });
    return NextResponse.json(item, { status: 201 });
  } catch (err) {
    console.error('POST /api/measurements', err);
