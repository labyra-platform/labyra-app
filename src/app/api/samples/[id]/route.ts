/**
 * /api/samples/[id] — read, update, deprecate (soft delete) a sample.
 *
 * DELETE here = deprecate (lifecycleStatus → 'deprecated').
 * For scientific retraction, use POST /api/samples/[id]/retract.
 *
 * @phase R164-phase-4a
 */
...
import { getSample, updateSample, deprecateSample } from '@/lib/firebase/samples/service';
...
export async function GET(req: NextRequest, ctx: RouteContext) {
  ...
  const item = await getSample(auth.tenantId, id);
...
export async function PATCH(req: NextRequest, ctx: RouteContext) {
  ...
  try {
    const updated = await updateSample(id, parsed.data, {
    ...
    console.error('PATCH /api/samples/[id]', err);
...
export async function DELETE(req: NextRequest, ctx: RouteContext) {
  ...
  try {
    await deprecateSample(id, auth.tenantId, auth.uid, reason);
