...
/** Per-row kebab for a chemical (R210). View + soft-delete with Undo. */
...
export function ChemicalsRowActions({ id, name }: { id: string; name: string }) {
  const t = useTranslations('chemicals');
  const router = useRouter();
  const locale = useLocale();
  const [busy, setBusy] = useState(false);

  const reactivate = async (cid: string) => {
    try {
      const token = await authToken();
      const res = await fetch(`/api/chemicals/${cid}/reactivate`, {
        method: 'POST',
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align='end'>
        <DropdownMenuItem onClick={() => router.push(`/${locale}/dashboard/chemicals/${id}`)}>
