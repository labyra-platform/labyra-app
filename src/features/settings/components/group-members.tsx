...
export function GroupMembers() {
  ...
  if (loading) {
    return (
      <Panel title={t('title')} icon={Icons.teams} className='max-w-2xl'>
  ...
  if (error) {
    return (
      <Panel title={t('title')} icon={Icons.teams} className='max-w-2xl'>
        <p className='text-muted-foreground text-body flex items-center gap-2 py-4'>
  ...
  if (!data?.group) {
    return (
      <Panel title={t('title')} icon={Icons.teams} className='max-w-2xl'>
        <PanelEmpty title={t('noGroup')} description={t('noGroupDesc')} />
      </Panel>
    );
  }
  ...
  return (
    <Panel
      title={data.group.name || t('unnamedGroup')}
      description={t('membersCount', { count: data.items.length })}
      icon={Icons.teams}
      className='max-w-2xl'
    >
      {data.items.length === 0 ? (
