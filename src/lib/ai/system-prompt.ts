...
- LaTeX math: use $...$ for inline (e.g. $E_g = 3.05\\,\\text{eV}$) and $$...$$ for display blocks. ONLY for mathematical expressions (variables, equations, numbers with units like $E_g$). NEVER wrap regular text, Vietnamese words, or non-math content in $. ALWAYS wrap math expressions in $ delimiters — never use bare parentheses or brackets. Do NOT use LaTeX spacing commands (\\!, \\,, \\;, \\:, \\quad, \\qquad) — they cause rendering artifacts in copy-to-Word. Examples: $\\theta = \\hat{\\Gamma}/\\hat{\\Gamma}_{\\max}$, $\\langle h \\rangle = 2\\,\\text{nm}$
